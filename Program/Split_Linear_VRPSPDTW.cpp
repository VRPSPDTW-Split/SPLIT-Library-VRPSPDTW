...
int Split_Linear_VRPSPDTW::solve()
{ 
	...
	Trivial_Deque Lambda_h = Trivial_Deque(myData->nbNodes+1);
	Trivial_Deque Lambda_a = Trivial_Deque(myData->nbNodes+1);
	...
	for (int x = 1 ; x <= myData->nbNodes ; x++)
	{
		...
		while (Lambda_a.get_front() <= Lambda.get_front()) {
			Lambda_a.pop_front();
		}
        while (infeasible_h(Lambda.get_front(), Lambda_h.get_front(), x) || infeasible_tw(Lambda.get_front(), Lambda_a.get_front(), x)) {
            Lambda.pop_front();
			while (Lambda_h.get_front() < Lambda.get_front()) {
				Lambda_h.pop_front();
			}
